!function(username, password, logging) {
    ...
    if (!(apikey && apisecret)) {
      ...
      lfm.auth.getMobileSession({username: username, password: password}, {
        success: function (data) {
          lsess = data.session
          cti = window.setInterval(checkTrack, 5000)
          sci = window.setInterval(scrobble, 20000)
        }
  ...
  window.stopjamming = function () {
    window.clearInterval(cti)
    window.clearInterval(sci)
  }
